...
####DateField.auto_now (when created-just once)
...
- `DecimalField(max_digit= 4 , decimal_paces = 3`: represents number upto 9 with decimal palces of 3 (3.456)
...
- modeledy by timedelta: `class datetime.timedelta(days=0, seconds=0, microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0)`
...
- in DB: created as **`varchar`** columns with default of `max_length=100` (can be overriden)
...
####through
- speicify **intermediary** with `through = 'some_table'`
...
####db_constraint 
- whether or not there should be contraint for intermediary (default: T)
